...
<aside class="bg-[#1E293B] fixed left-0 top-0 h-screen w-[260px] border-r border-slate-700/50 shadow-2xl flex flex-col py-6 z-50 hidden md:flex">
    ...
    <nav class="flex-1 overflow-y-auto space-y-0.5 px-2">
        ...
        <?= navLink('dashboard',  $base.'/index',     'dashboard',    'Dashboard',    $currentPage) ?>
        <?= navLink('clients',    $base.'/clientes',  'group',        'Clientes',     $currentPage) ?>
        <?= navLink('services',   $base.'/servicos',  'build_circle', 'Serviços',     $currentPage) ?>

        <p class="text-[9px] font-bold text-slate-500 uppercase tracking-widest px-4 pt-4 pb-2">Faturação</p>
        <?= navLink('quotes',     $base.'/orcamentos',    'request_quote', 'Orçamentos',   $currentPage) ?>
        <?= navLink('create_quote',$base.'/criar-orcamento', 'post_add',    'Criar Orçamento',$currentPage) ?>
        <?= navLink('invoices',   $base.'/faturas',       'description',  'Faturas',      $currentPage) ?>
        <?= navLink('create_inv', $base.'/criar-fatura',  'add_circle',   'Criar Fatura', $currentPage) ?>
        <?= navLink('payments',   $base.'/pagamentos',    'payments',     'Pagamentos',   $currentPage) ?>

        <p class="text-[9px] font-bold text-slate-500 uppercase tracking-widest px-4 pt-4 pb-2">Análise</p>
        <?= navLink('reports',    $base.'/relatorios',    'bar_chart',    'Relatórios',   $currentPage) ?>

        <?php if (($_SESSION['user_role'] ?? '') === 'admin'): ?>
        <p class="text-[9px] font-bold text-slate-500 uppercase tracking-widest px-4 pt-4 pb-2">Sistema</p>
        <?= navLink('users',      $base.'/utilizadores',  'people',     'Utilizadores',  $currentPage) ?>
        <?= navLink('settings',   $base.'/configuracoes', 'settings',   'Configurações', $currentPage) ?>
        <?php endif; ?>
    </nav>

    <!-- User info & Logout -->
    <div class="px-4 mt-4 border-t border-slate-700/50 pt-4">
        <a href="<?= $base ?>/perfil" class="flex items-center gap-3 px-2 mb-3 group/user hover:bg-slate-800/50 p-2 rounded-xl transition-all">
